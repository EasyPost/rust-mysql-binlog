...
fn parse_maybe_inlined_value(cursor: &mut Cursor<Vec<u8>>, compound_size: CompoundSize) -> Result<(u8, OffsetOrInline), Error> {
    let t = cursor.read_u8()?;
    let inlined_value = match FieldType::from_byte(t) {
        Ok(FieldType::Literal) => {
        ...
        Ok(FieldType::Uint16) => {
            JsonValue::from(cursor.read_u16::<LittleEndian>()?)
        },
        Ok(FieldType::Int16) => {
            JsonValue::from(cursor.read_i16::<LittleEndian>()?)
        },
        Ok(FieldType::Uint32) => {
            JsonValue::from(cursor.read_u32::<LittleEndian>()?)
        },
        Ok(FieldType::Int32) => {
            JsonValue::from(cursor.read_i32::<LittleEndian>()?)
        },
        Ok(_) | Err(_) => {
            return Ok((t, OffsetOrInline::Offset(match compound_size {
...
fn parse_any_with_type_indicator(mut cursor: &mut Cursor<Vec<u8>>, type_indicator: FieldType) -> Result<JsonValue, Error> {
    match type_indicator {
        ...
        FieldType::Custom => {
            ...
            match column_type {
                ColumnType::NewDecimal(..) | ColumnType::Date | ColumnType::Time | ColumnType::Timestamp | ColumnType::DateTime | ColumnType::DateTime2(..) | ColumnType::Time2(..) | ColumnType::Timestamp2(..) => {
                ...
                _ => {
                    ...
                    m.insert("column_type".to_owned(), JsonValue::from(raw_mysql_column_type));
                    m.insert("base64_payload".to_owned(), JsonValue::from(serialized_payload));
                    Ok(JsonValue::from(m))
                }
            }
        }
...
mod tests {
    ...
    #[test]
    pub fn test_nested() {
        let blob = vec![0u8, 1, 0, 46, 0, 11, 0, 1, 0, 2, 12, 0, 97, 4, 0, 34, 0, 5, 1, 0, 5, 2, 0, 12, 16, 0, 0, 22, 0, 5, 116, 104, 114, 101, 101, 1, 0, 12, 0, 11, 0, 1, 0, 5, 4, 0, 52];
        let parsed = parse(blob).expect("should parse");
    ...
    #[test]
    pub fn test_array() {
        let blob = vec![2, 5, 0, 21, 0, 4, 1, 0, 4, 2, 0, 4, 0, 0, 5, 0, 0, 12, 19, 0, 1, 48];
        let parsed = parse(blob).expect("should parse");
    ...
    pub fn test_opaque_times() {
        let blob = vec![0, 4, 0, 97, 0, 32, 0, 4, 0, 36, 0, 4, 0, 40, 0, 8, 0, 48, 0, 9, 0, 15, 57, 0, 15, 67, 0, 15, 77, 0, 15, 87, 0, 100, 97, 116, 101, 116, 105, 109, 101, 100, 97, 116, 101, 116, 105, 109, 101, 116, 105, 109, 101, 115, 116, 97, 109, 112, 10, 8, 0, 0, 0, 0, 0, 188, 159, 25, 11, 8, 0, 0, 0, 64, 218, 0, 0, 0, 12, 8, 0, 0, 0, 64, 218, 188, 159, 25, 7, 8, 0, 0, 0, 77, 218, 188, 159, 25];
        let parsed = parse(blob).expect("should parse");
        assert_eq!(parsed, json!({"date": null,"datetime":{"DateTime":{"day":7,"hour":82,"minute":69,"month":78,"second":44,"subsecond":0,"year":184640201}},"time":{"Time":{"hours":0,"minutes":0,"seconds":0,"subseconds":0}},"timestamp":{"Timestamp":{"subsecond":0,"unix_time":1291845632}}}));
    }
}
